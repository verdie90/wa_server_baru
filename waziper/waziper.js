...
const WAZIPER = {
	...
	instance: async function(access_token, instance_id, res, callback){
		var verify_response = {"status":"success"}
		var verified = true;
	...
	webhook: async function(instance_id, data){
		var tb_webhook = Common.db_query("SHOW TABLES LIKE 'sp_whatsapp_webhook'");
		if(tb_webhook){
			var webhook = Common.db_query("SELECT * FROM sp_whatsapp_webhook WHERE status = 1 AND instance_id = '"+instance_id+"'");
			if(webhook){
